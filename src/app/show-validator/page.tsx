...
import { Banknote, LucideBinary, LucideCoins, LucideHandCoins, LucideLock } from 'lucide-react';
...
export default function ValidatorCard() {
	...
	useEffect(() => {
		async function fetchValidatorData() {
			try {
				...
				const validatorTotalStake: any = await Promise.all(
					sortedValidators.map(async (validator: any) => {
				...
				const validatorSelfStake: any = await Promise.all(
					sortedValidators.map(async (validator: any) => {
				...
				const lockedStake = await Promise.all(
					sortedValidators.map(async (validator: any) => {
				...
				const validatorDelegatedStake: any = await Promise.all(
					sortedValidators.map(async (validator: any) => {
				...
				const validatorInfo: any = await Promise.all(
					sortedValidators.map(async (validator: any) => {
						...
						return {
							...additionalData,
							validator,
							totalStake: totalStakeData.totalStake,
							selfStake: validatorSelfStakeData.selfStake,
							delegatedStake: DelegatedData.delegatedStake,
							lockedStake: lockedStakeData.lockedStake
						};
					})
				);
	...
	const defaultImage =
		'https://gotbit-vinu-update-validator-backet-dev.s3.us-east-1.amazonaws.com/validators/vita_inu.png';
	const defaultContact = 'vinuscan.com';
	const defaultName = 'Unknown';

	return (
		<main className="flex min-h-screen flex-col items-center justify-center p-5 text-black">
			<h1 className="mb-2 text-4xl font-bold">Delegate</h1>
			<p className="mb-4 text-lg">It's time to choose the validator you'd like to support.</p>
			<span className="mb-6 text-sm">TIP: Click on a validator's name to learn more about them!</span>
					...
					<Card key={index} className="w-80">
						...
						<CardContent className="flex flex-col gap-3">
							<p className="flex items-center gap-3">
								<LucideBinary />{' '}
								{Number(validator.validator) ? Number(validator.validator) : 'VC Validator'}
							</p>
							<p className="flex items-center gap-3">
							...
							<p className="flex items-center gap-3">
								<LucideHandCoins />{' '}
								{validator.delegatedStake ? formatNumber(validator.delegatedStake) : 'VC Delegated'}
							</p>
							<p className="flex items-center gap-3">
